...
while True:
    ...
    if score >= score_full:
        ...
        time_text = font_large.render("Your time is {0:.2f} sec".format(time[0] - time_press_cont[0]), True, WHITE)
        time_rect = time_text.get_rect()
        time_rect[0], time_rect[1] = screen_x / 2 - time_rect[2] / 2, screen_y / 2 - time_rect[3] / 2
        pygame.draw.rect(screen, DARKER_GREEN, time_rect)
        ...
        continue_text = font_large.render("Restart", True, WHITE)
        ...
        if continue_rect.colliderect(image_pos_x, image_pos_y, 1, 1) and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            gif_number = 0
            score = 0
            jump_or_fall = "jump"
            image_bunny_all_rect[0] = screen_x + 1
            time = []
            time_press_cont = []
            time_press_cont.append(float(pygame.time.get_ticks()) / 1000)
